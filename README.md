...
- 一键复制 cURL
- 设置面板支持代理配置（HTTP/SOCKS，地址/端口/用户名/密码）
- 自动识别本地服务端口并生成请求地址
...
- **复制 cURL**：点击 **复制 cURL**。
- **代理配置**：在 `Settings | HTTP Client` 里配置 HTTP/SOCKS 代理，发送请求和远程导入 OpenAPI/Swagger 时会生效。
...
- 前后置脚本升级为多作用域执行模型：请求执行时按 **全局 -> 项目 -> 模块 -> 接口** 顺序聚合脚本，自动跳过未启用或空脚本。
- 新增全局/项目/模块脚本列表管理：支持新增、删除、编辑、启用/禁用、顺序调整，并支持同一作用域下多个脚本同时生效。
- 接口级脚本保持单脚本模型，新增“接口前置脚本/接口后置脚本”独立启用开关，可按需参与执行。
- 新增代理配置能力：可在 `Settings | HTTP Client` 配置 HTTP/SOCKS 代理（地址、端口、用户名、密码），用于请求发送与远程 OpenAPI/Swagger 导入。
- 脚本上下文新增 `endpoint` 变量：对代码来源接口提供方法注解映射、参数类型与参数注解映射、方法体、方法描述符、类描述符；非代码来源默认空 Map 或 `null`。
- 前后置脚本管理弹窗重构为 **左侧列表 + 右侧编辑器**，并优化启用列宽，脚本维护更直观。
- 新增全局/项目/模块三种脚本作用域图标，提升工具栏入口可辨识度。

### v0.0.4

- 新增接口变量能力：可在请求页维护变量，并通过 `{{name}}` / `{{api.name}}` / `{{project.name}}` / `{{global.name}}` / `{{env.name}}` / `{{path.id}}` 进行模板替换。
- 新增变量匹配可视化：URL 与 value 列支持匹配状态高亮和悬浮提示，可查看变量来源、解析值与未匹配原因。
- URL 路径占位符联动增强：支持 `{id}` 与路径变量实时匹配，并修复首次打开请求时匹配状态延迟刷新的问题。
- 请求工具栏新增 **环境变量** 入口，可直接维护项目环境/全局环境变量，无需切换到脚本页。
- 接口文档增强：请求文档新增“路径变量说明”和“查询参数说明”卡片，自动汇总对应页签中的字段说明与示例值。
- 草稿持久化优化：编辑请求参数后自动保存当前 Tab，避免重新打开后值回退。
